...
# CC Nav Status — ワーカー環境の状態確認

## 手順

1. `ccws ls` で全ワーカー一覧を取得
2. 各ワーカーディレクトリで以下を収集:
   - 現在のブランチ名
...
4. ワーカーが0件の場合: 「ワーカーはありません。`ccws new <name> <branch>` で作成できます。」と表示

## 注意

- `ccws` コマンドが見つからない場合は `/opt/homebrew/bin/ccws` または `~/.cargo/bin/ccws` を試す
